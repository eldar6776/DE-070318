...
#define MPU6050							           //定义使用的传感器是MPU6050
#define MOTION_DRIVER_TARGET_MSP430		 //采用MSP430的驱动,移植到STM32F42x
...
u8 mpu_dmp_init(void)
{
	...
	if(mpu_init()==0)	              
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);           //设置所需传感器
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);      //设置FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	                 //设置采样率
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		                  //装载dmp固件
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//陀螺仪方向控制
		if(res)return 5; 
...
/**
  * @brief  得到dmp处理后的数据
  * @param  pitch:俯仰角 精度:0.1°   
  * @param  roll:横滚角  精度:0.1°   
  * @param  yaw:航向角   精度:0.1°   
  * @retval 0:成功;其他:失败
  */
