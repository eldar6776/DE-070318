...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
uint8_t i2c_ee_buffer[I2C_EE_BUFFER_SIZE];
...
int16_t I2C_EERPOM_ReadBytes(uint8_t slave_address, uint8_t data_address, uint8_t cnt)
{
	...
	if(cnt == 0) return(-8);
	if(cnt > I2C_EE_BUFFER_SIZE) return(-9);
	I2C_GenerateSTART(I2C2, ENABLE);
	timeout = I2C2_TIMEOUT;
	...
	for(n = 0; n < cnt; n++) 
	{
		...
		i2c_ee_buffer[n] = I2C_ReceiveData(I2C2);
	}
...
int16_t I2C_EERPOM_WriteBytes(uint8_t slave_address, uint8_t data_address, uint8_t cnt)
{
	...
	if(cnt == 0) return(-6);
	if(cnt > I2C_EE_BUFFER_SIZE) return(-7);
	I2C_GenerateSTART(I2C2, ENABLE);
	timeout = I2C2_TIMEOUT;
	...
	for(n = 0; n < cnt; n++) 
	{
		I2C_SendData(I2C2, i2c_ee_buffer[n]);
		timeout = I2C2_TIMEOUT;
