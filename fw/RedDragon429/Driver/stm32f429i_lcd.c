...
#define FONT_8x16   0
#define FONT_16x24  1
extern  unsigned char WB_LOGO[];//19206
...
/* LTDC Initialization -------------------------------------------------------*/
  ...
  LTDC_Layer_InitStruct.LTDC_VerticalStop = 12+ 240 -1;// 
  ...
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = (uint32_t)LCD_BUF;
  ...
  */
  LTDC_Layer_InitStruct.LTDC_CFBPitch = ( 320* 2);  /*320x240为图片宽度和高度320*/
  ...
  LTDC_Layer_InitStruct.LTDC_HorizontalStart = 360 +  43;//
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = (459  + 43 - 1); //
  LTDC_Layer_InitStruct.LTDC_VerticalStart = 106 + 12;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = (203 + 12 - 1);
  ...
  LTDC_Layer_InitStruct.LTDC_ConstantAlpha =255;// 180;
  ...
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = (uint32_t)(&WB_LOGO);
  ...
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((99 * 2) + 3); 
  LTDC_Layer_InitStruct.LTDC_CFBPitch = (99 * 2);
  
  /*  the pitch is the increment from the start of one line of pixels to the 
  start of the next line in bytes, then :
  Pitch = Active high width x number of bytes per pixel     
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineNumber = 97; 
...
/**
  ...
  * @param  Xpos: specifies the X position, can be a value from 0 to 480.
  * @param  Ypos: specifies the Y position, can be a value from 0 to 272.
  * @param  Height: display rectangle height, can be a value from 0 to 272.
  * @param  Width: display rectangle width, can be a value from 0 to 480.
  * @retval None
  */
...
/**
  ...
  * @param  x,y :??????
  * @param  num: ????? 
  * @param  PenColor,BackColor:????,????
  * @param  font:0:8x16; 1:16x24 ??????????????
  * @retval None
  */
...
{   
    ...
    if( font==FONT_8x16){                                            //??????8*16???       
        ...
        for(pos=0;pos<CHAR_H;pos++) {
            ...
            for(t=0;t<CHAR_W;t++) {
                if(temp&0x80){			                                 //?????                   		
    				PutPixelColor(x+t, y+pos, PenColor);                     //????? ???            
    			} else {			
    				PutPixelColor(x+t, y+pos, BackColor);	                   //????? ???            
                }     
	            temp<<=1;
            }        
        }      
