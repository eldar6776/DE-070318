...
void CreateDefaultDisplay(void)
{
	...
	GUI_SetTextMode(GUI_TEXTMODE_TRANS);	
	GUI_GotoXY(190, 20);
	GUI_DispString("Hotel Hills");
	GUI_SetFont(&GUI_Font16B_1);
	GUI_SetColor(GUI_YELLOW);
	GUI_GotoXY(25, 5);
	GUI_DispString("CONGRESS & TERMAL");
	GUI_GotoXY(25, 30);
	GUI_DispString("SPA RESORT  *  *  *  *  *");
	GUI_SetFont(&GUI_Font32B_1);
	GUI_GotoXY(200, 0);
	GUI_DispString("*  *  *  *  *");
	GUI_SetColor(GUI_WHITE);
...
void TestCameraInterface(void)
{
	...
	camm_type = OV7670_Check();
	
	if (camm_type == 0x7673)
	{
		if (OV7670_Init())while (1) continue;
	}
	else                                        
	{
		camm_type = OV2640_Check();
		
		if(camm_type == 0x2642) 
		{
			if (OV2640_Init()) while (1) continue;
		}
		else
		{
			if (ov5642_init()) while (1) continue;
		}
	}
