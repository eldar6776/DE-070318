...
//#define TOUCH_AD_TOP     295  	// left
//#define TOUCH_AD_BOTTOM  1765	// right
//#define TOUCH_AD_LEFT    326  	// top
//#define TOUCH_AD_RIGHT   1685 	// bottom

#define TOUCH_AD_TOP     0  
#define TOUCH_AD_BOTTOM  272
#define TOUCH_AD_LEFT    0  
#define TOUCH_AD_RIGHT   480
//
...
*/
  ...
  #define DISPLAY_DRIVER_1   GUIDRV_LIN_16 //GUIDRV_LIN_OSX_16
#elif (COLOR_MODE_1 == _CM_ARGB1555)
  #define COLOR_CONVERSION_1 GUICC_M1555I
  #define DISPLAY_DRIVER_1   GUIDRV_LIN_OSX_16 //GUIDRV_LIN_16
...
//static const U32    		_aAddr[] 	= { LCD_FRAME_BUFFER_test, LCD_FRAME_BUFFER_test + XSIZE_PHYS * YSIZE_PHYS * sizeof(U32) * NUM_VSCREENS * NUM_BUFFERS };
...
static int _GetBytesPerLine(int LayerIndex, int xSize) {
  ...
  return BytesPerLine;        //return 480
}
...
static void _DMA_LoadLUT(LCD_COLOR * pColor, U32 NumItems) {
  ...
  //
  DMA2D->FGPFCCR  = LTDC_Pixelformat_RGB565         // Pixel format
...
static void _LTDC_SetLayerPos(int LayerIndex, int xPos, int yPos) {
  ...
  HorizontalStart = xPos + HBP + 1;//43;
  HorizontalStop  = xPos + HBP + xSize;//480+43-1;
...
static void _LCD_InitController(int LayerIndex) {
	...
	LTDC_Layer_InitStruct.LTDC_HorizontalStart = HBP;//43;
	LTDC_Layer_InitStruct.LTDC_HorizontalStop  = (xSize + HBP-1);//480+43-1;//
	LTDC_Layer_InitStruct.LTDC_VerticalStart   = VBP;//12;//
	LTDC_Layer_InitStruct.LTDC_VerticalStop    = (ySize + VBP-1);//272+12-1;//
	//
	//  配置像素格式
	//
	Pixelformat = _GetPixelformat(LayerIndex);
	...
	// Reload configuration
	//
	LTDC_ReloadConfig(LTDC_SRCR_IMR);//刷新使能
}
...
static void _LCD_DrawBitmap16bpp(int LayerIndex, int x, int y, U16 const * p, int xSize, int ySize, int BytesPerLine) {
  ...
  OffLineSrc = (BytesPerLine / 2) - xSize;//480
...
void LCD_X_Config(void)
{
	...
	GUI_TOUCH_Calibrate(GUI_COORD_Y, 0, YSIZE_PHYS, TOUCH_AD_LEFT, TOUCH_AD_RIGHT);	
  #if (GUI_NUM_LAYERS > 1)
    //
	...
	//GUI_TOUCH_Calibrate(GUI_COORD_Y, 0, 272, TOUCH_AD_LEFT, TOUCH_AD_RIGHT);	
  #endif
	//
