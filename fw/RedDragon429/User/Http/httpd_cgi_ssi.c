...
const char * HTTP_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
    ...
    if (iIndex == 0)
    {
		...
		/* Check cgi parameter for RUBICON udate display image:			SET /sysctrl.cgi?iuf;iul;ifa;ila	*/
		for (i = 0; i < iNumParams; i++) 
		{
		...
		/* Check cgi parameter for RUBICON set room temperature:		SET /sysctrl.cgi?tsp 	*/
		for (i = 0; i < iNumParams; i++) 
		{
			...
			{  
				...
				*/
				...
				if(rs485_interface_address == (t & 0xffff))
				{
					...
					I2C_EERPOM_Delay(I2C_EE_WRITE_DELAY);
