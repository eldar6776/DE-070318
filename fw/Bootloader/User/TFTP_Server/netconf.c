...
void LwIP_Init(void)
{
  ...
  memp_init();
...
#ifdef USE_LCD  
  	iptab[0] = NETMASK_ADDR3;
	iptab[1] = NETMASK_ADDR2;
	iptab[2] = NETMASK_ADDR1;
	iptab[3] = NETMASK_ADDR0;

	sprintf((char*)sbnttxt, "  %d.%d.%d.%d", iptab[3], iptab[2], iptab[1], iptab[0]);
	...
	sprintf((char*)gtwtxt, "  %d.%d.%d.%d", iptab[3], iptab[2], iptab[1], iptab[0]);	
...
//  LCD_DisplayStringLine(LCD_LINE_8, (uint8_t*)"  Static IP address   ");
//  LCD_DisplayStringLine(LCD_LINE_9, iptxt);
#endif
#endif
